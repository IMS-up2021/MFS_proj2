include "Io.dfy"

// Bounded Naive String Matching
// Searches for a pattern exclusively within a specified segment of the text array.
method ContainsWord(txt: array<byte>, txtStart: int, txtEnd: int, pat: array<byte>) returns (found: bool)
    // Precondition: We cannot meaningfully search for an empty pattern.
    requires pat.Length > 0

    // Precondition: Establishes a strict mathematical sandbox. The start and end indices 
    // must be valid non-negative numbers, must be logically ordered, and must not exceed the text length.
    requires 0 <= txtStart <= txtEnd <= txt.Length
{
    var len := txtEnd - txtStart;
    var M := pat.Length;

    if len < M { 
        return false; 
    }

    var i := 0;
    while i <= len - M
        // As the read head `i` increments, the remaining length `len - i` strictly decreases.
        decreases len - i

        // Loop Invariant (Bounds): `i` will never exceed the point where the remaining 
        // characters in the chunk are fewer than the pattern length. 
        // The `+ 1` is necessary because `i` increments at the end of the loop before the condition is checked.
        invariant 0 <= i <= len - M + 1
    {
        var j := 0;
        while j < M
            // On a match, `j` increments, decreasing `M - j`. On a mismatch, the `break` 
            // statement immediately terminates the loop, satisfying Dafny's requirements.
            decreases M - j

            // Loop Invariant (Bounds): `j` stays strictly within the bounds of the pattern length.
            invariant 0 <= j <= M

            // Loop Invariant (Crucial Memory Safety): This proves we never read past the 
            // designated chunk (the line). Since the maximum of `i` is `len - M`, and the 
            // maximum of `j` is `M`, `i + j` maxes out at `len`. 
            // Therefore, `txtStart + len` simplifies exactly to `txtEnd`.
            invariant txtStart + i + j <= txtEnd // Ensures we never read past the designated line end
        {
            if txt[txtStart + i + j] != pat[j] {
                break;
            }
            j := j + 1;
        }

        if j == M {
            return true;
        }
        i := i + 1;
    }
    return false;
}

// Output Helper
// Safely converts bytes to a string sequence to avoid literal character printing.
method PrintBufferSegment(buffer: array<byte>, start: int, end: int)
    // Precondition: The chunk boundaries must be within the bounds of the buffer.
    requires 0 <= start <= end <= buffer.Length
{
    var s: string := [];
    var k := start;
    while k < end
        // Termination Metric: `k` strictly approaches `end`.
        decreases end - k
        // Loop Invariant: `k` safely stays within the provided bounds.
        invariant start <= k <= end
    {
        // Ignore the carriage return byte (13) to clean up Windows line endings
        if buffer[k] != 13 {
            s := s + [buffer[k] as char];
        }
        k := k + 1;
    }
    print s;
    print "\n";
}

// Line Parsing and Execution
// Scans the file for newlines and processes each line individually.
method GrepLines(buffer: array<byte>, pat: array<byte>)
    // Precondition: Must have a valid pattern to pass down to `ContainsWord`.  
    requires pat.Length > 0
{
    var i := 0;
    var line_start := 0;

    while i < buffer.Length
        // Precondition: Must have a valid pattern to pass down to `ContainsWord`.
        decreases buffer.Length - i

        // Loop Invariant (Bounds): `i` safely traverses the buffer.
        invariant 0 <= i <= buffer.Length

        // Loop Invariant (Logic Safety): The start of the current line can never surpass 
        // the current read head `i`. The `+ 1` accounts for when we set `line_start := i + 1` 
        // after finding a newline.
        invariant 0 <= line_start <= i + 1
    {
        // When we hit a newline byte (10)
        if buffer[i] == 10 {
            // Safety Check: Verifies the chunk bounds are strictly logical before passing them to ContainsWord.
            if line_start <= i {
                var found := ContainsWord(buffer, line_start, i, pat);
                if found {
                    PrintBufferSegment(buffer, line_start, i);
                }
            }
            // Advance the start of the next line to the character immediately following the newline.
            line_start := i + 1;
        }
        i := i + 1;
    }

    // Process the final chunk of the file if it doesn't end with a trailing newline
    if line_start <= buffer.Length {
        var found := ContainsWord(buffer, line_start, buffer.Length, pat);
        if found {
            PrintBufferSegment(buffer, line_start, buffer.Length);
        }
    }
}

// The Main Executable
method {:main} Main(ghost env: HostEnvironment?)
    // Precondition (I/O State): Ensures the host environment is initialized and hasn't crashed.
    requires env != null && env.Valid() && env.ok.ok()

    // Modification Framing: Allows the method to alter the success state (`env.ok`) and file system.
    modifies env.ok
    modifies env.files
{
    var numArgs := HostConstants.NumCommandLineArgs(env);
    if numArgs != 3 {
        print "Usage: ./grep-naive <Word> <File>\n";
        return;
    }

    var wordArg := HostConstants.GetCommandLineArg(1, env);
    var fileArg := HostConstants.GetCommandLineArg(2, env);

    if wordArg.Length == 0 {
        print "Error: Search word cannot be empty.\n";
        return;
    }

    var fileExists := FileStream.FileExists(fileArg, env);
    if !fileExists { return; } // UNIX grep fails silently if file doesn't exist

    var success, len := FileStream.FileLength(fileArg, env);
    if !success || len < 0 { return; }

    var ok, f := FileStream.Open(fileArg, env);
    if !ok { return; }

    var buffer := new byte[len];
    ok := f.Read(0 as nat32, buffer, 0 as int32, len);
    if !ok { return; }
    
    var closeOk := f.Close();

    var wordBytes := new byte[wordArg.Length];
    var k := 0;
    while k < wordArg.Length
        invariant 0 <= k <= wordArg.Length
    {
        var charVal := wordArg[k] as int;
        wordBytes[k] := if 0 <= charVal < 256 then charVal as byte else 63; 
        k := k + 1;
    }

    // Execute the UNIX-style line matching
    GrepLines(buffer, wordBytes);
}